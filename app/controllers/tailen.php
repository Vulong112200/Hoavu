			...
			switch ($act) {
				...
				case 'addmv':{
					...
					if($_FILES["image2"]["name"]){
						$image2 = "public/images/Bài hát-MV/".time().$_FILES["image2"]["name"];
						move_uploaded_file($_FILES["image2"]["tmp_name"], "public/images/Bài hát-MV/".time().$_FILES["image2"]["name"]);
					}
					if($_FILES["mv"]["name"]){
						$link2 = "public/mv/".time().$_FILES["mv"]["name"];
						move_uploaded_file($_FILES["mv"]["tmp_name"], "public/mv/".time().$_FILES["mv"]["name"]);
					}

					$this->Model->execute("insert into mvtl(AnhMV, id, tenMV, Link) values('$image2','$id','$name2', '$link2')");
